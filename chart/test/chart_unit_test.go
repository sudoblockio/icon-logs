...
func TestIconBlocksUnit(t *testing.T) {
	...
	apiOutput := helm.RenderTemplate(t, options, helmChartPath, "deployment-icon-logs*", []string{"templates/deployment-api.yaml"})
	workerOutput := helm.RenderTemplate(t, options, helmChartPath, "deployment-icon-logs*", []string{"templates/deployment-worker.yaml"})
